...
public class uSVGTransform {
  ...
  private uSVGTransformType _type;
  private uSVGMatrix _matrix;
  ...
  public uSVGTransform() {
    this._matrix = new uSVGMatrix();
    this._type = uSVGTransformType.SVG_TRANSFORM_MATRIX;
  }
  ...
  public uSVGTransform(string strKey, string strValue) {
    string[] valuesStr = uSVGStringExtractor.ExtractTransformValue(strValue);
    ...
    switch(strKey) {
      ...
      break;
      default:
        this._type = uSVGTransformType.SVG_TRANSFORM_UNKNOWN;
      break;
    }
  }
  //***********************************************************************************
  ...
  public void SetTranslate(float tx, float ty) {
    this._type = uSVGTransformType.SVG_TRANSFORM_TRANSLATE;
    this._matrix = new uSVGMatrix().Translate(tx, ty);
  }

  public void SetScale(float sx, float sy) {
    this._type = uSVGTransformType.SVG_TRANSFORM_SCALE;
    this._matrix = new uSVGMatrix().ScaleNonUniform(sx, sy);
  }

  public void SetRotate(float angle) {
    this._type = uSVGTransformType.SVG_TRANSFORM_ROTATE;
    this._angle = angle;
    this._matrix = new uSVGMatrix().Rotate(angle);
  }

  public void SetRotate(float angle, float cx, float cy) {
    this._type = uSVGTransformType.SVG_TRANSFORM_ROTATE;
    this._angle = angle;
    this._matrix = new uSVGMatrix().Translate(cx, cy).Rotate(angle).Translate(-cx,-cy);
  }

  public void SetSkewX(float angle) {
    this._type = uSVGTransformType.SVG_TRANSFORM_SKEWX;
    this._angle = angle;
    this._matrix = new uSVGMatrix().SkewX(angle);
  }

  public void SetSkewY(float angle) {
    this._type = uSVGTransformType.SVG_TRANSFORM_SKEWY;
    this._angle = angle;
    this._matrix = new uSVGMatrix().SkewY(angle);
  }
}