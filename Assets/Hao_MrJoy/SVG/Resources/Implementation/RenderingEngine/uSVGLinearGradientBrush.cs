...
public class uSVGLinearGradientBrush {
  ...
  private void GetStopList() {
    List<uSVGStopElement> _stopList = this._linearGradElement.stopList;
  ...
  private void SetGradientVector(uSVGGraphicsPath graphicsPath) {
    ...
    if(this._linearGradElement.x1.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
      this._x1 = bound.x +(bound.width * this._x1 / 100f);
    }

    if(this._linearGradElement.y1.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
      this._y1 = bound.y +(bound.height * this._y1 / 100f);
    }

    if(this._linearGradElement.x2.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
      this._x2 = bound.x +(bound.width * this._x2 / 100f);
    }

    if(this._linearGradElement.y2.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
      this._y2 = bound.y +(bound.height * this._y2 / 100f);
    }

    if(this._linearGradElement.gradientUnits == uSVGGradientUnitType.SVG_OBJECT_BOUNDING_BOX) {
