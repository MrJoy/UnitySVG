...
public class SVGRadialGradientBrush {
  ...
  private void GetStopList() {
    List<SVGStopElement> _stopList = this._radialGradElement.stopList;
  ...
  private void SetGradientVector(SVGGraphicsPath graphicsPath) {
    ...
    if(this._radialGradElement.cy.unitType == SVGLengthType.Percentage) {
    ...
    if(this._radialGradElement.fx.unitType == SVGLengthType.Percentage) {
      this._fx = bound.x +(bound.width * this._fx / 100f);
    }
    if(this._radialGradElement.fy.unitType == SVGLengthType.Percentage) {
