...
public class uSVGRadialGradientBrush {
	...
	private void f_Initialize() {
		this.m_cx = this.m_radialGradElement.cx.animVal.value;
		this.m_cy = this.m_radialGradElement.cy.animVal.value;
		this.m_r = this.m_radialGradElement.r.animVal.value;
		this.m_fx = this.m_radialGradElement.fx.animVal.value;
		this.m_fy = this.m_radialGradElement.fy.animVal.value;
	...
	private void f_FixF() {
		if (Mathf.Sqrt((this.m_fx-this.m_cx) * (this.m_fx-this.m_cx)) + ((this.m_fy-this.m_cy) * (this.m_fy-this.m_cy)) > this.m_r) {
	...
	private float f_Percent(float x, float y) {
		...
		//float d1 = Mathf.Sqrt((m_cP.x - x) * (m_cP.x - x) + (m_cP.y - y) * (m_cP.y - y));
		float d2 = Mathf.Sqrt((this.m_fx - x) * (this.m_fx - x) + 
								(this.m_fy - y) * (this.m_fy - y));
		float dd = Mathf.Sqrt((m_cP.x - this.m_fx) * (m_cP.x - this.m_fx) + 
	...
	private void f_SetGradientVector(uSVGGraphicsPath graphicsPath) {
		...
		if (this.m_radialGradElement.cx.animVal.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
			this.m_cx = bound.x + (bound.width * this.m_cx / 100f);
		}

		if (this.m_radialGradElement.cy.animVal.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
			this.m_cy = bound.y + (bound.height * this.m_cy / 100f);
		}

		if (this.m_radialGradElement.r.animVal.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
			...
			float dd = Mathf.Sqrt((m_p2.x - m_p1.x) * (m_p2.x - m_p1.x) + 
										(m_p2.y - m_p1.y) * (m_p2.y - m_p1.y));
			this.m_r = (dd * this.m_r / 100f);
		}
		
		if (this.m_radialGradElement.fx.animVal.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
			this.m_fx = bound.x + (bound.width * this.m_fx / 100f);
		}
		if (this.m_radialGradElement.fy.animVal.unitType == uSVGLengthType.SVG_LENGTHTYPE_PERCENTAGE) {
			this.m_fy = bound.y + (bound.height * this.m_fy / 100f);
		}


		if (Mathf.Sqrt((this.m_cx - this.m_fx) * (this.m_cx - this.m_fx) +
